...
class PackageControllerTest {
    ...
    void testCreatePackage() {
        ...
        when(packageService.save(packageDTO)).thenReturn(packageDTO);

        ResponseEntity<PackageDTO> response = packageController.createPackage(packageDTO);

        assertEquals(HttpStatus.CREATED, response.getStatusCode());
        assertEquals(packageDTO, response.getBody());
        verify(packageService, times(1)).save(packageDTO);
    }
